...
pacman::p_load(tidyverse, lubridate, showtext, ggExtra)
...
**3 Engineering of Categorical (`fct`) and Numerical Features (`dbl`, `int`)**
>3.1 Engineering of Categorical Features  
3.2 Engineering of Numerical Features  
3.3 Handling of Extreme Values 
3.4 Handling of Missing Values

**4 Excursus: Data leakage**

**5 Feature selection**  
> 5.1 Intrinsic Methods  
5.2 Filter and Wrapper Methods
...
- know important techniques for detecting and handling outlier as well as missing values,<br><br>
...
**Introductory Example:** Have you ever thought about the numerous potential ways of how to represent a date feature (`2020-09-09`) in your machine learning pipeline? There are more options than you would think at first glance...
<br><br><br>
**Option 1:** Number of days since *unix time* (`1970-01-01`)
```{r, echo=F}
difftime(as.Date("2020-09-09"), as.Date("1970-01-01"), units="days")
```
**Option 2:** Year, month and day of the week feature
```{r, echo=F}
print(c(lubridate::year("2020-09-09"), lubridate::month("2020-09-09"), lubridate::wday("2020-09-09")))
```
**Option 3:** Day of the year
```{r, echo=F}
lubridate::yday("2020-09-09")
```
**Option 4:** Boolean period indicator (`2020-06-29` < x < `2020-08-11`)
```{r, echo=F}
between(as.Date("2020-09-09"), as.Date("2020-06-29"), as.Date("2020-08-11"))
```

---

## 3.1 Engineering of Categorical Features (`fct`)
### 3.1.1 Encoding of Unordered Features
...
## 3.1 Engineering of Categorical Features (`fct`)
### 3.1.2 Encoding of Ordered Features
...
Theoretically, a polynomial contrast can model most non-linear shape in the relationship between the response and the categorical feature of up to degree `C-1`. Yet, in practice the exploration of polynomial contrasts does rarely exceed polynomials of degree 2 [[1]](#references).
...
## 3.1 Engineering of Categorical Features (`fct`)
### 3.3.3 Aggregation of Near-zero Variance Features
...
**Rule-of-thumb for near-zero variance features:** `(#zeros / #ones) > 19` [[1]](#references)
...
If a feature is considered a near-zero variance feature, a potential remedy is to lump together rare feature levels into an "other" category (e.g., using ``forcats::fct_lump()`).
...
## 3.2 Engineering of Numerical Features (`dbl`, `int`)
### 3.2.1 (1:1)-Transformations

**Log-transformation** and **inversion**: Resolves skewness and shifts the distribution closer to a Gaussian.

```{r, echo=F, out.height='35%', out.width='35%', fig.retina=3, fig.align='center', out.extra='style="float:left; padding:10px"'}
penguins %>% 
  drop_na %>% 
  ggplot() +
...
.pull-left[
```{r, echo=F, out.height='70%', out.width='70%', fig.retina=3, fig.align='center'}
penguins %>% 
  drop_na %>% 
...
The removal of outlier must be guided by the employed model and is context-dependent.
...
A good idea is to run the analysis with and without outlier and compare the results. However, in order to do so, the outlier must be identified in the first place.
<br><br><br>
...
## 3.3 Handling of Extreme Values
### 3.3.2 Outlier Detection `r emo::ji("graph")`

.pull-left[
```{r, echo=F, out.height='70%', out.width='70%', fig.retina=3, fig.align='left'}
penguins %>% 
  drop_na %>% 
...
- *Outlier:* Points beyond the hinges
]
...
.pull-left[
```{r, echo=F, out.height='70%', out.width='70%', fig.retina=3, fig.align='center', messages=F}
p <- penguins %>% 
  drop_na %>% 
...
.pull-right[
**Scatterplots with marginal densities:** Convenient tool for identifying outlier in two dimensions.
- Even if a data point is quite extreme with respect to one predictor, its value with respect to other predictors can be entirely unremarkable.<br><br>
- Outlier detection in higher dimensions requires techniques which take the whole feature space into account.
]

.pull-right[
.footnote[
*Note: `ggplots`with marginal densities can be generated by combining `ggplot2` with the `ggExtra` package.*
]]

???
scatterplot with marginal densities:
- nice tool to identify outlier with regards to two dimensions

---

## 3.3 Handling of Extreme Values
### 3.3.2 Outlier Detection `r emo::ji("abacus")`

Numerical approaches for outlier detection and correction usually involve some form of *truncation* or *trimming* (i.e. elimination). That is, extreme values are either capped respectively artificially lowered or entirely removed from the data set to reduce their influence on the predictive model.
...
.pull-right[
```{r, echo=F, out.height='75%', out.width='75%', fig.retina=3, fig.align='center', messages=F, warnings=F}
...
penguins_pred %>% 
  ggplot(aes(x = x, y = y)) +
    ...
    geom_point(
      ...
      aes(color = "High leverage"), size = 4, shape = 1) + 
    geom_point(
      data = penguins_pred %>% slice_min(x, n = 3),
      aes(color = "High leverage"), size = 4, shape = 1) + 
    geom_point(
      data = penguins_pred %>% slice_max(resid, n = 5),
      aes(color = "Large residual"), size = 4, shape = 1) + 
...
.footnote[.pull-left[
*Note: You can find multiple references in the literature with regards to the optimal cutoff threshold for Cook's distance. Commonly, a value of 1 is applied as a threshold for truncation respectively trimming.*
]]
...
Finally, also the machine learning community has provided novel tools for detecting and handling outliers in a model-driven manner. A prominent approach is the use of **isolation trees** (resp. **isolation forests** [[5]](#references)).

.pull-left[
**Main intuition:**
- Due to their uniqueness, outlier are separated early on the partitioning procedure. Hence, they are closer to the *root* of the tree.
- The fewer splits required to isolate a sample (i.e. the shorter the *path length*), the more likely the sample is an outlier.
- By ranking each observations by its path length, outlier can be identified.
]
.pull-right[
...
**Decision rule:** Relatively short (long) path length indicate outlier (normal samples). Relatively homogeneous path length across all samples indicate the absence of outlier.
...
## 3.4 Handling of Missing Values
### 3.4.2 Missing Value Detection

.pull-left[
**Missing value heatmap:** `r emo::ji("graph")`
```{r, echo=F, out.height='75%', out.width='75%', fig.retina=3, fig.align='left'}
penguins %>%
  mutate(
...
## 3.4 Handling of Missing Values
### 3.4.3 Missing Values: Handling of Random `NA`
...
**Robust algorithms:** Use algorithms that are robust against the presence of `NA`. For example, the *CART* algorithm for decision trees effectively handles missing values by enabling surrogate splits. Alternatively, [[7]](#references) discusses outlier robust estimators in the context of linear regression.
...
## 3.4 Handling of Missing Values
### 3.4.3 Missing Values: Handling of Non-Random `NA`
...
Techniques for feature transformation (e.g., z-normalization), outlier correction (e.g., winsorization) or handling missing values (e.g., mean-impuation) **must always be embedded in your resampling approach**!
...
.footnote[
*Note: Revisit online lecture 5 on resampling methods (in particular slides 17-21).  
[[8]](#references-2) and [[9]](#references-2) are also great resources for getting an intuitive understanding of data leakage.*
]
...
## 5 Feature Selection
### 5.1 Intrinsic Methods `r emo::ji("package")`

Intrinsic methods provide embedded feature selection functionalities. This broadly refers to all methods that have some type of regularization built-in, such as Lasso or pruned decision trees.
...
## 5 Feature Selection
### 5.2 Filter Methods `r emo::ji("magnifying glass tilted right")` and Wrapper Methods `r emo::ji("counterclockwise arrows button")`

.pull-left[
**Filter methods** [[10]](#references-2) discard features based on domain knowledge or statistical indicators, e.g., correlations, chi-square scores, p-values or odds-ratios. These methods are
...
.pull-right[
**Wrapper methods** [[10]](#references-2) wrap the process of model training and feature selection into a for-loop and successively discard irrelevant features. These methods are
...
???
filter methods: often correlation type measures
...
[8]: **Brownlee, J. (2016):** Data Leakage in Machine Learning. Machine Learning Mastery 2016-08-02 (updated 2020-08-15). URL: https://machinelearningmastery.com/data-leakage-machine-learning/.

[9]: **Data Skeptic (2017):** [MINI] Leakage. Data Skeptic Podcast 2017-05-10. URL: https://www.youtube.com/watch?v=dI5oGNTi6Ys.

[10]: **Kohavi, R./George, H. J. (1997):** Wrappers for Feature Subset Selection. Artificial intelligence Vol. 97, 1997, No. 1-2, pp. 273-324.

[11]: **Olah, C./Mordvintsev, A./Schubert, L. (2017):** Feature Visualization: How Neural Networks Build up Their Understanding of Images. URL: https://distill.pub/2017/feature-visualization/

[12]: **Mikolov, T., et al. (2013):** Distributed Representations of Words and Phrases and Their Compositionality. NIPS 2013. URL: http://papers.nips.cc/paper/5021-distributed-representations-of-words-and-phrases-and-their-compositionality.pdf.

[13]: **Dai, A. M./Olah, C./Le, Q. V (2015):** Document Embedding with Paragraph Vectors. arXiv Working Paper 2015-07-29. URL: https://arxiv.org/pdf/1507.07998.pdf.
