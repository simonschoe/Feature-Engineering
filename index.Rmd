...
pacman::p_load(xaringanthemer, tidyverse, lubridate, showtext, ggExtra)
...
```{r, echo=F}
print(c(lubridate::year("2020-11-23"), lubridate::month("2020-11-23"), lubridate::wday("2020-11-23")))
```
...
Theoretically, a polynomial contrast can model most non-linear shapes in the relationship between the response and the categorical feature of up to degree $C-1$. Yet, in practice the exploration of polynomial contrasts does rarely exceed polynomials of degree 2 [[1]](#references).
...
**Rule-of-thumb for zv features:** `(#zeros / #ones) > 19` [[1]](#references)
```{r, echo=F}
penguins %>% 
  count(island) %>% 
...
$\textbf{z}$**-normalization:** Modifies the scale of the predictor

$$Z=\frac{X-\mu } {\sigma}$$

The $z$-normalization involves two steps: *centering* (subtracting the mean) and *scaling* (dividing by the standard deviation). Afterwards $Z$ follows a standard normal distribution with  $\tilde{}N(0, 1)$.

--

<br><br>
**min/max-scaling:** Modifies the scale of the predictor

$$X'=  \frac{X - min(X)}{max(X) - min(X))}$$

Min/max-scaling restricts data points to the $[0; 1]$-interval. A generalization of min/max-scaling is **range scaling** where the data points are projected onto an arbitrary interval.
...
**Basis expansion:** Establish non-linear relationships between a predictor and the response by adding polynomials (e.g., quadratic and cubic terms)
```{r, eval=F}
penguins %>% 
  dplyr::mutate(
...
.pull-right[
In certain cases, binning helps to reduce complexity of the relationship between $X$ and $Y$ and hence facilitates the interpretation.
...
- **Box area:** inter-quartile range ( $IQR$ ), corresponds to the $[Q_{0.25}; Q_{0.75}]$-interval<br><br>
...
???
assumption here: you have no option of acquiring the missing values ex post. that should always be the first approach if it affordable.

---

## 5 Excursus: Data Leakage `r emo::ji("droplet")`

**Intuition:** Pre-processing techniques, such as feature transformation (e.g., $z$-normalization), outlier correction (e.g., winsorization) or missing values handling (e.g., mean-imputation), depend on the concrete train-test-split (i.e. the training observations) as part of the validation set or cross-validation approach:

- $z$*-normalization:* The mean and standard deviation depends on your data set.<br><br>
...
.footnote[
*Note: Revisit online lecture 5 on resampling methods (in particular slides 17-21). [[8]](#references-2) and [[9]](#references-2) are also great resources for getting an intuitive understanding of data leakage.*
...
Intrinsic methods provide embedded feature selection capabilities. This broadly refers to all methods that have some type of regularization built-in, such as Lasso regression or pruned decision trees.
...
.footnote[*Source: [[12]](#references-2)*]
]
.pull-right[
```{r, echo=F, fig.align='center', out.height='80%', out.width='80%'}
...
.footnote[*Source: [[13]](#references-2)*]
]
